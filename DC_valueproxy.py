...
def model_test(fig_name, train_data, test_data, basis=rbf.basis.se, order=2, penalty=0.4, size=500, x_dim = 5, y_dim = 1, each_y_dim = 1):
	'''
	对RBF网络的的测试函数，
...
if __name__ == '__main__':
	...
	order_list1 = [2]
	penalty_list1 = [0.15]
	size_list1 = [200, 300, 500, 700, 800, 900]

	number_list = [ele for ele in range(10)]
	...
	print(raw_data['input'][0])

	all_index_list = [temp for temp in range(len(raw_data['input']))]
	
	#收集测试集的数据集合
	test_data_index = sample(all_index_list, 100)
	test_data = {'input':[], 'label':[], 'index':[]}
	test_data['input'] = np.array(raw_data['input'], dtype=np.float64)[test_data_index, :]
	test_data['label'] = np.array(raw_data['label'], dtype=np.float64)[test_data_index, :]
	test_data['index'] = np.array(raw_data['index'], dtype=np.int32)[test_data_index]
	...
	for kernel in basis_dict.items():
		for order in order_list1:
			for penalty in penalty_list1:
				for index, size in enumerate(size_list1):
					if index == 0:
						temp = sample(all_index_list, size)
					else:
						temp = sample(all_index_list, size - size_list1[index-1])
					...
					for ele in temp:
						if ele in all_index_list:
							...
							pass
					train_data_index = train_data_index + temp

					train_data = {'input':[], 'label':[], 'index':[] }
					train_data['input'] = np.array(raw_data['input'], dtype=np.float64)[train_data_index, :]
					train_data['label'] = np.array(raw_data['label'], dtype=np.float64)[train_data_index, :]
					train_data['index'] = np.array(raw_data['index'], dtype=np.int32)[train_data_index]
					...
					for number in number_list:
						...
						temp = model_test(fig_name, train_data, test_data, kernel[1], order, penalty, size)
						...
						#csv_writer.writerow(specific_data)
