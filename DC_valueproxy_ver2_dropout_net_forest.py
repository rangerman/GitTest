...
def model_test(fig_name, raw_data, basis=rbf.basis.se, order=2, penalty=0.4, size=500, x_dim = 5, y_dim = 1, each_y_dim = 1):
	...
	print("*************************>get ready for dropout RBF net train<**********************")

	all_net = []
	net_num = 20
	for net_index in range(net_num):
		current_wanted_index = sample(wanted_data_index, int(len(wanted_data_index) * 0.75) ) #以75%作为dropout的提取率
		train_set = {'input':[], 'label':[], 'index':[] }
		train_set['input'] = np.array(raw_data['input'], dtype=np.float64)[current_wanted_index, :]
		train_set['label'] = np.array(raw_data['label'], dtype=np.float64)[current_wanted_index, :]
		train_set['index'] = np.array(raw_data['index'], dtype=np.int32)[current_wanted_index]
	...
	accuracy = 0
	#针对测试集中100个样本，每个样本都单独过一次net森林
	for i in range ( len(test_set_label) ):
		#针对当前元素位置i,获取其对应的真实comparison_list
		real_comparison_list = get_comparison_list(test_set_label, i)
		#针对net森林，对于任一元素而言，其predict_comparison_list情况应该是由net森林投票决定的
		predict_comparison_list = 0

		for net_index in range( net_num ):
			#获取net森林中任一net子树的评估list
			test_set_predict = all_net[net_index]( test_set_inputs )
			test_set_predict = test_set_predict[:, 0].reshape(-1)

			predict_comparison_list += np.array( get_comparison_list(test_set_predict, i), dtype=np.float64 )

		predict_comparison_list /= net_num
		final_item_comparison_list = []
		#投票表决制，大于50%的投票则认可为1
		for item in predict_comparison_list:
			if item >= 0.5:
				final_item_comparison_list.append(1)
			else:
				final_item_comparison_list.append(0)

		iter_accuracy = compare_accuracy(real_comparison_list, final_item_comparison_list)

		accuracy += iter_accuracy
		print("index {0} element comparison accuracy is {1}".format(i, iter_accuracy) )
...
if __name__ == '__main__':
	...
	output_csv_path = "5_31_rapid_comparison_dropout.csv"
	output = open(output_csv_path, 'a+', newline='')
